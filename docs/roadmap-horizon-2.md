...
- **B6 (draft engine, engine half) — done.** `packages/schema/src/draft.ts`: pure, seeded pick-1-of-3 flow reusing `@opencards/core`'s existing RNG; a completed draft's decklist satisfies `validateDecklist` by construction (proven across 3 seeds). UI half (pack-opening/pick screen) is the natural next slice.
