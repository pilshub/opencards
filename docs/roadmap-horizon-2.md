...
- **B5 (windfury keyword) — done.** Reused existing `attacksThisTurn` plumbing; `getLegalCommands` needed zero changes thanks to the apply()-filter pattern. Engine-only, no card uses it yet.
- **B6 (draft — engine + UI) — done.** `packages/schema/src/draft.ts`: pure, seeded pick-1-of-3 flow reusing `@opencards/core`'s existing RNG; a completed draft's decklist satisfies `validateDecklist` by construction (proven across 3 seeds). `packages/app/src/components/Draft.tsx` drives it end to end (real cards, real active format, Save writes to the same storage the hot-seat deck editor reads). Verified live in a real browser against the real build.
